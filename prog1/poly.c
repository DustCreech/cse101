...
void polyOrder(List degree, List coeff){
    ...
    while (length(degree) > 0){ //iterate until we reach an empty list
...
void polyPrint(List degree, List coeff, char var){
    for(int i = 0; i != length(degree); i++){ //iterate until we reach the length of the list
	int degVal = delElement(degree, 0); 
	int coefVal = delElement(coeff, 0);
	appendList(degree, degVal);
	appendList(coeff, coefVal);
	if (i == 0){ //case in order to not let a plus sign be first
	    if (coefVal < 0 && coefVal != -1){ //for a negative value
	        if (degVal > 1){
		    printf("-%d%c^%d ", coefVal * -1, var, degVal); //multiply by 1 in order for it to be positive
		} else if (degVal == 1){
		    printf("-%d%c ", coefVal * -1, var);
		} else {
		    printf("-%d ", coefVal * -1);
		}
	    } else if (coefVal == 1 || coefVal == -1){ //we do not want to print a 1 as the coeff.
		if (coefVal == -1){
		    if (degVal > 1){ //if the degree is greater than 1 we want the ^
                        printf("-%c^%d ", var, degVal);
                    } else if (degVal == 1){ //if its just one we only want the variable
                        printf("-%c ", var);
                    } else { //0th degree which we only want the integer value
                        printf("-%d ", coefVal * -1);
                    }
		} else {
		    if (degVal > 1){
	    ...
	    } else { //the coeff value is positive
		if (degVal > 1){
	...
	} else if (i == length(degree) - 1){ //we know we are at the last term and allows us to print a new line after
	...
	} else { //we are on one of the middle terms
	    if (coefVal < 0 && coefVal != -1){
                if (degVal > 1){
...
void polySimplify(List degree, List coeff, char var){
    List newDeg = newList();
    List newCoeff = newList();
    int sum; //keeps track of the like degree's coeffs.
	...
	if (findIndex == -1){ // there is no more terms of the same degree
	    if (sum == 0){ //the sum cancels one another out
	        sum = 0;
	    } else {
	        appendList(newDeg, degVal);
	        appendList(newCoeff, sum);
	        sum = 0; //reseting the sum
	    }
	} 
    }
    polyPrint(newDeg, newCoeff, var); //prints the simplified poly
    freeList(&newDeg); //freeing the lists
    freeList(&newCoeff);
}
...
void polyMult(List degree, List coeff, List degree2, List coeff2, char var){
    ...
    for( int i = count; i != 0; i--){ //iterates through the lists in order to add the degree and mult. the coeff.
        int degVal = delElement(degree, 0); //deleting the 0th element due to the next element always moving to the 0th index
        int coefVal = delElement(coeff, 0);
        for(int j = count2; j != 0; j--){
            int deg2Val = delElement(degree2, 0);
            int coef2Val = delElement(coeff2, 0);
            appendList(multDegree, deg2Val + degVal); //appending the sum of the two terms degrees
            appendList(multCoeff, coef2Val * coefVal); //appending the product of the two terms coeffs.
            appendList(cpyDegree, deg2Val);
            appendList(cpyCoeff, coef2Val);
        }
        degree2 = cpyDegree; //setting the original lists as the copies
        coeff2  = cpyCoeff;
    }
    polyPrint(multDegree, multCoeff, var); //printing the sum of the two polys product
    printf("which simplifies to\n");
    polySimplify(multDegree, multCoeff, var); //calling to simplify the new poly
    //freeing all the lists created
    freeList(&multDegree); 
    freeList(&multCoeff);
    freeList(&cpyDegree);
    freeList(&cpyCoeff);
...
int main(void){
    ...
    int MAX_LEN = 1000;
    char *line = (char *) calloc(MAX_LEN, sizeof(char)); //allows us to get the poly 
    ...
    line2[len2-1] = '\0';
    char lineVar;
    for (int i = 0; i < len; i++){ //gets the var from the poly
    ...
    //gets the degree and coeff. lists for the two polys
    getLists(lineDegree, lineCoeff, len, line);
    getLists(line2Degree, line2Coeff, len2, line2);

    //orders the polys into the correct way
    ...
    printf("\n");
    //freeing all the lists and the lines
    freeList(&lineDegree);
    freeList(&lineCoeff);
    freeList(&line2Degree);
    freeList(&line2Coeff);
    free(line);
    free(line2);
    return 0;
